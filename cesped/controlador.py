...
class Controlador:
    def __init__(self, nombre, periodo_ms=1000, f_lector=utils.identidad, f_calibrador=utils.identidad, f_envio=utils.identidad):
        ''' nombre:       string
            f_lector:     f() -> D
            f_calibrador: f(D) -> X
            periodo_ms:   int
        ...
        self.task         = tr.Tarea(self,self.f_envio,periodo_ms)
    ...
    def obtener_dato(self):
        "devuelve un dato leido mediante `self.f_lector' y tratado con `f_calibrador'"
    ...
    def componer_dato(self,dato):
        ...
        return {'ts': utils.timestamp(),
                 self.name : dato}
    
    def enviar_dato(self):
        "obtiene (`obtener_dato') y envía un dato (usando `f_envio')"
        
        envio = self.componer_dato(self.obtener_dato())
        return self.f_envio(envio)
        
    nombre       = "None"
    f_lector     = utils.identidad
    f_calibrador = utils.identidad
    periodo_ms   = 1000
    f_envio      = utils.identidad
    task         = None
